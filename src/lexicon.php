...
class Lexicon
{
     ...
     * If file has .tree extension it is assumed
     * that file is serialized Lexicon
     *
     * @param string $filename
     * @return Lexicon
     */
    public static function fromFile($filename) : Lexicon
    {
        $fileParts = explode('.', $filename);
        if (array_pop($fileParts) === 'tree') {
            echo "Unserializing lexicon...";
            $lexicon = unserialize(file_get_contents($filename));
        } else {
    ...
    private function _leftSearch(Board &$board, stdClass $anchor, $word, array $rack, stdClass &$lexicon, $limit)
    {
        ...
        foreach ($lexicon as $letter => &$node) {
            ...
            if (in_array('?', $rack)) {
                ...
                unset($rack[$index]);
                $return = array_merge($return, $this->_leftSearch($board, $anchor, $word . mb_strtolower($letter), $rack, $node, $limit - 1));
                $rack[$index] = '?';
            }
        }
        ...
        if (!$board->isUsed($anchor->row, $col)) {
            ...
            foreach ($lexicon as $letter => &$target) {
                ...
                if (in_array('?', $rack)) {
                    $index = array_search('?', $rack);
                    unset($rack[$index]);
                    $return = array_merge($return, $this->_rightSearch($board, $anchor, $word . mb_strtolower($letter), $rack, $target, $col + 1));
                    $rack[$index] = '?';
                }
