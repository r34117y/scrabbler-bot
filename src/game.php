...
abstract class Game
{
    ...
    public static $DISTRIBUTION_PL = array(
        '?' => 2,
        'A' => 9,
        'B' => 2,
        'C' => 3,
    ...
    public function __construct(array $options = array())
    {
        ...
        // Step 6: Fill Bag
        $this->bag = array();
        foreach ($this->getDistribution() as $letter => $count) {
    ...
    public function executeCommand($command)
    {
        ...
        // Get new letters
        $new_letters = mb_str_split($new_letters);
    ...
    public function simulate(Game $opponent, $meFirst = true)
    {
        ...
        $racks = array(mb_str_split($previous[0][0]), str_split($previous[1][0]));
        ...
        for ($index = (int)!$meFirst; ; $index = (int)!$index) {
            try {
                ...
                if (!empty($previous[$index][0])) {
                    $racks[$index] = array_merge($racks[$index], mb_str_split($previous[$index][0]));
                }
